...
import type { CategoryTotals } from "@/lib/itinerary-data";
    ...
    <section className="rounded-3xl border border-border bg-card p-6 shadow-sm">
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className={cn("font-bold", over ? "text-warning" : "text-foreground")}>
            {usedPct}%
          </span>
        </div>
      ...
      {over ? (
        <div className="mt-4 flex items-start gap-2 rounded-2xl border border-warning/30 bg-warning-muted p-3 text-sm">
          ...
          <p className="font-medium text-foreground">
            Your itinerary is <span className="text-warning">{inr(Math.abs(remaining))}</span> over
            budget.
          </p>
        </div>
      ) : approaching ? (
        ...
        <div className="mt-4 flex items-start gap-2 rounded-2xl border border-primary/20 bg-accent p-3 text-sm">
          ...
          <p className="font-medium text-foreground">
            You&apos;re comfortably within budget. Nice planning!
          </p>
        </div>
      )}
