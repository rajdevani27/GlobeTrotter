  ...
  head: () => ({
    meta: [
      { title: "Itinerary Builder | GlobeTrotter" },
      {
        name: "description",
        content:
          "Build a day-by-day itinerary with activities, budget tracking, hotel and restaurant suggestions.",
      },
      { property: "og:title", content: "Itinerary Builder | GlobeTrotter" },
      {
        property: "og:description",
        content: "Plan your day-by-day trip itinerary and track your budget.",
      },
    ],
  }),
...
function ItineraryPage() {
  ...
  function deleteActivity(day: number, id: string) {
    setDays((prev) =>
      prev.map((d) =>
        d.day === day ? { ...d, activities: d.activities.filter((a) => a.id !== id) } : d,
      ),
    );
          ...
          {hasItinerary ? (
            <div className="flex items-center gap-2">
              ...
              <Button
                size="sm"
                className="h-9"
                onClick={() => setToast("Itinerary saved to your trips")}
              >
        ...
        {!hasItinerary ? (
          <section className="mt-8 flex flex-col items-center rounded-3xl border border-dashed border-border bg-card/50 px-6 py-14 text-center">
            ...
            <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground text-pretty">
              ...
              <span className="font-medium text-foreground">Generate itinerary</span> to see a
              personalized day-by-day plan, budget breakdown and route preview.
            </p>
            <Button
              size="lg"
