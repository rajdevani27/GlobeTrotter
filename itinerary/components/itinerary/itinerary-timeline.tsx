  ...
  return (
    <div className="flex flex-col gap-8">
      {days.map((day) => {
        ...
        return (
          <section key={day.day} className="rounded-3xl border border-border bg-card/60 p-5 shadow-sm sm:p-6">
            <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
              <div className="flex items-center gap-3">
                <div className="flex size-12 flex-col items-center justify-center rounded-2xl bg-primary text-primary-foreground">
                  <span className="text-[0.6rem] font-semibold uppercase leading-none opacity-80">Day</span>
                  <span className="font-display text-lg font-bold leading-none">{day.day}</span>
                </div>
                <div>
                  <h3 className="font-display text-lg font-bold text-foreground text-pretty">{day.title}</h3>
            ...
            <div className="relative mt-5">
              <span className="absolute bottom-4 left-2.5 top-2 w-px bg-border sm:left-3.5" aria-hidden="true" />
