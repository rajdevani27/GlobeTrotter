  ...
  return (
    <section className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8">
      ...
      <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-muted-foreground">
        Tell us where you&apos;re headed and what you love — we&apos;ll craft a smart day-by-day plan you can
        fully customize.
      </p>
        ...
        <Field label="Travelers" htmlFor="travelers">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => onChange({ ...trip, travelers: Math.max(1, Number(e.target.value) || 1) })}
              className={cn(inputClass, "pl-9")}
            />
          </div>
        </Field>
        ...
        <Field label="Budget (₹)" htmlFor="budget" className="sm:col-span-2 lg:col-span-1">
          <div className="relative">
            ...
            <input
              ...
              step={1000}
              value={trip.budget}
              onChange={(e) => onChange({ ...trip, budget: Math.max(0, Number(e.target.value) || 0) })}
              className={cn(inputClass, "pl-9")}
            />
          </div>
        </Field>
      ...
      <div className="mt-7 flex flex-col gap-3 border-t border-border pt-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button size="lg" onClick={onGenerate} disabled={isGenerating} className="h-11 px-6 text-sm">
          {isGenerating ? (
            <>
